...
test_optimal_assign_layer :: proc(t: ^testing.T) {
	tests: []OptimalAssignTest = {
		{name = "Empty layer", n_slots = 3, items = {}, want_cols = {}},
		{
...
run_optimal_assign_test :: proc(t: ^testing.T, tt: OptimalAssignTest) {
	...
	for item, i in tt.items {
		ok := graph_node_add(&graph, u64(i + 1))
		testing.expect(
			t,
			ok,
			fmt.tprintf("[%s] Failed to add node %d", tt.name, i + 1),
		)
	...
	optimal_assign_layer(&graph, layer_start, layer_end, tt.n_slots)
...
test_assign_columns_integration :: proc(t: ^testing.T) {
	...
	graph: Graph
	graph_init(&graph, 10, 6, {100, 50}, context.allocator)
	...
	for edge in edges {
		...
		testing.expect(
			t,
			ok,
			fmt.tprintf("Failed to add edge %d -> %d", edge[0], edge[1]),
		)
	}
	...
	expected := map[u64]u16 {
		1 = 0,
		2 = 0,
		3 = 0, // Chain 1 all in column 0
		5 = 1,
		6 = 1, // Chain 2 all in column 1
		7 = 2,
		8 = 2,
		9 = 2, // Chain 3 all in column 2
	}
